...
export default function ProjectPage() {
  ...
  const fetchData = async (
    ...
    try {
      const url = `${process.env.NEXT_PUBLIC_API_BASE}/project/index?page=${page}&pageSize=${pageSize}&searchQuery=${searchQuery}&selectedFilter=${selectedFilter}`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE}/project/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="d-flex gap-2">
          {/* {perms.includes("edit project") ? ( */}
            <button
              ...
              onClick={() => router.push(`/hrm/projects/edit?id=${row.id}`)}
            >
              <i className="bi bi-pencil"></i> Edit
            </button>
          {/* ) : null} */}

          {/* {perms.includes("delete project") ? ( */}
            <button
  ...
  // if (!perms.includes("view project")) {
