                ...
                <aside className="widget-area extra-padding">
                  <div className="widget widget_search">
                    <form className="search-form" onSubmit={(e) => e.preventDefault()}>
                      <input type="search" className="search-field" placeholder="Search posts..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ width: "100%", padding: "10px", border: "1px solid #ddd", borderRadius: "6px", outline: "none" }} />
                    ...
                    {filteredPosts.map((post) => (
                      <article className="item" key={post.id} style={{ paddingBottom: "20px", marginBottom: "20px", borderBottom: "1px solid #eee" }}>
                        ...
                        <div style={{ fontWeight: "400", marginTop: "8px" }}>
                          <Link href={`/blog/${post.slug}`} style={{ fontWeight: "400", color: "#000", textDecoration: "none", fontSize: "14px", lineHeight: "1.4" }}>{post.name}</Link>
                        </div>
                      </article>
                    ))}
                  </div>
